...
const Category = () => {
    ...
    useEffect(() => {
        const fetchCategoryProducts = async () => {
            try {
                ...
                setError(t('dashboard.category.error'));
                setLoading(false);
    ...
    if (loading) {
        return <div className="text-center py-3">{t('dashboard.category.loading')}</div>;
    }
    ...
    if (categoryProducts.length === 0) {
        return <div className="text-center py-3">{t('dashboard.category.empty')}</div>;
    }

    return (
        <div className="category-section my-4 p-5  ">
            <h2 className="mb-4   fw-bold text-white">{t('dashboard.category.title')}</h2>
            <Row>
