...
const ProductsHot = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setError(t('dashboard.productsHot.error'));
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="text-center py-3">{t('dashboard.productsHot.loading')}</div>;
  }
  ...
  return (
    <div className="Hot-Products my-4 p-5">
      ...
      {products.length > 0 && (
        <div className="mt-5">
          <h2 className="text-warning fw-bold mb-4">{t('dashboard.productsHot.title')}</h2>
          <Row>
